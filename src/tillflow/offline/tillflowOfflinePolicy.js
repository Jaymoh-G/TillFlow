 ...
 * sales and quoting flows that are ready for offline use. All other /admin
 ...
 *
 * POS register lives under /pos (separate layout) and is always reachable offline.
 */
...
export const TILLFLOW_ADMIN_SALES_NAV_PATH_RE =
  /\/admin\/(orders|quotations|leads|proposals|invoices|delivery-notes|invoice-payments|sales-returns)(\/|$)/;

/** Admin routes that may be used without network today (excludes invoicing until built). */
const ADMIN_OFFLINE_CAPABLE_RE = /\/admin\/(orders|quotations|sales-returns|purchases|purchase-orders|purchase-returns)(\/|$)/;
