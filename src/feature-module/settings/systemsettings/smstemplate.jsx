...
const Smstemplate = () => {
  ...
  const persistAll = useCallback(() => {
    ...
    n.smsTemplates = templates;
    saveSystemSettings(n);
    setSavedMsg("SMS templates saved.");
    window.setTimeout(() => setSavedMsg(""), 3500);
  }, [templates]);

  const resetOne = useCallback((id) => {
    const d = getDefaultSystemSettings().smsTemplates[id];
    setTemplates((t) => ({ ...t, [id]: { ...d } }));
  }, []);

  const reloadSmsTemplatesFromServerCache = useCallback(() => {
    setTemplates({ ...loadSystemSettings().smsTemplates });
    setSavedMsg("");
  }, []);
  useReloadFromTenantUiSettingsHydration(reloadSmsTemplatesFromServerCache);
              ...
              <div className="settings-wrapper d-flex">
                ...
                <div className="card flex-fill mb-0 min-w-0">
                  <div className="card-header d-flex flex-wrap align-items-start justify-content-between gap-2">
                    <div>
                      <h4 className="mb-0">SMS templates</h4>
                      {isTillflow ? (
                        <p className="text-muted small mb-0">Short messages — localStorage; keep under ~300 chars for single SMS.</p>
                      ) : null}
                    </div>
                              ...
                              <div className="addproduct-icon d-flex align-items-center justify-content-between w-100">
                                <div className="d-flex align-items-center">
                                  <div className="status-toggle modal-status d-flex justify-content-between align-items-center me-2">
                                    ...
                                    />
                                    <label htmlFor={`st-${def.id}`} className="checktoggle" />
                                  </div>
                                  <h5 className="mb-0">
                                    <span>{def.title}</span>
                                  </h5>
