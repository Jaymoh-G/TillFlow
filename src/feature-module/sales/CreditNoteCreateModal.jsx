...
import { mapInvoiceItemsToCreditCandidates } from "./creditNoteViewHelpers";
...
export default function CreditNoteCreateModal({ show, onHide, invoiceRow, saving, error, onSubmit }) {
  const [issuedAt, setIssuedAt] = useState("");
  const [notes, setNotes] = useState("");
  const [qtyByItemId, setQtyByItemId] = useState({});

  const candidates = useMemo(() => mapInvoiceItemsToCreditCandidates(invoiceRow?.items), [invoiceRow?.items]);
  ...
  const totalAmountPreview = useMemo(() => {
    return candidates.reduce((acc, row) => {
      const k = String(row.invoiceItemId ?? row.key);
      const qty = Number(qtyByItemId[k] ?? 0);
      if (!Number.isFinite(qty) || qty <= 0) {
        return acc;
      }
      return acc + Math.min(row.remainingQty, qty) * (Number(row.unitPrice ?? 0) || 0);
    }, 0);
  }, [candidates, qtyByItemId]);
  ...
  return (
    <Modal show={show} onHide={onHide} centered size="lg" backdrop={saving ? "static" : true}>
      <Modal.Header closeButton={!saving}>
        <Modal.Title>Generate Credit Note</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        ...
        <div className="row g-2 mb-3">
          ...
          <div className="col-md-8">
            ...
            <input
              ...
              placeholder="Reason or remarks for this credit note"
            />
          </div>
        </div>
                ...
                candidates.map((row) => {
                  ...
                  return (
                    <tr key={k}>
                      ...
                      <td className="text-end">{row.creditedQty}</td>
                      <td className="text-end">{row.remainingQty}</td>
                      <td className="text-end">{Number(row.unitPrice ?? 0).toFixed(2)}</td>
                      <td>
                        <input
                          type="number"
        ...
        <p className="small text-muted mb-0 mt-2">Preview total credit amount: Ksh {totalAmountPreview.toFixed(2)}</p>
      </Modal.Body>
      <Modal.Footer>
