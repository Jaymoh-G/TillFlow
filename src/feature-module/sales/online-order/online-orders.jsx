...
import { webOrderData } from "../../../core/json/webOrderData";
...
const OnlineOrder = () => {
  ...
  const [sourceRows] = useState(() => [...webOrderData]);
  ...
  const channelOptions = useMemo(() => {
    const ch = [...new Set(sourceRows.map((r) => r.channel))].sort((a, b) =>
      String(a).localeCompare(String(b))
    );
    return [ALL, ...ch.map((c) => ({ label: c, value: c }))];
  }, [sourceRows]);
  ...
  const displayRows = useMemo(() => {
    ...
    if (filterChannel) {
      list = list.filter((r) => r.channel === filterChannel);
    }
    if (filterStatus) {
  ...
  return (
    <div>
      <div
        className={`page-wrapper online-orders-page${
          inTillflowShell ? " online-orders-page--tillflow" : ""
        }`}>
          ...
          <div className="page-header">
            <div className="add-item d-flex">
              <div className="page-title">
                <h4>Online orders</h4>
                <h6>
                  Web and remote orders — search and filter by customer, channel, or
                  payment status.
                </h6>
              </div>
            </div>
            ...
            <div className="page-btn d-flex flex-wrap gap-2">
              ...
              {inTillflowShell ? (
                ...
                <Link to={all_routes.posorder} className="btn btn-outline-primary">
                  <i className="ti ti-device-laptop me-1" />
                  POS orders
                </Link>
              )}
              ...
              <div className="d-flex table-dropdown my-xl-auto right-content align-items-center flex-wrap row-gap-3 gap-2">
                ...
                <div style={{ minWidth: "10rem" }}>
                  <CommonSelect
                    ...
                    value={filterChannel === "" ? "" : filterChannel}
                    onChange={(e) => {
                      const v = e.value;
                      setFilterChannel(v == null || v === "" ? "" : String(v));
                    }}
                    placeholder="Channel"
                    filter={false}
                  />
